...
from dotabuff.query import DotaQuery, DotaSteamQuery


class Test_DotaQuery(unittest.TestCase):
    def setUp(self):
        self.queries = [DotaSteamQuery(), DotaQuery()]


    def test_players_by_name_happymeds(self):
        for query in self.queries:
            players = query.get_players("HappyMeds")
    ...
    def test_players_by_name_atomic(self):
        for query in self.queries:
            players = query.get_players("Atomic")
            self.assertTrue(len(players) > 0)
            atomic = players[0]
    ...
    def test_verified_players_first_if_any(self):
        for query in self.queries:
            players = query.get_players("Dendi")
            self.assertTrue(len(players) > 0)
            Dendi = players[0]
    ...
    def test_matches_of_a_player(self):
        for query in self.queries:
            matches = query.get_matches("88271237")
    ...
    def test_vip_players(self):
        for query in self.queries:
            for vip_player_name in VIP_PLAYERS_IDS:
                players = query.get_players(vip_player_name)
                self.assertTrue(len(players) > 0)
